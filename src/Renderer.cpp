...
void Renderer::CreateComputeDescriptorSetLayout() {
    ...
    VkDescriptorSetLayoutBinding numRemainingBladesLayoutBinding = {};
    numRemainingBladesLayoutBinding.binding = 2;
    numRemainingBladesLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    numRemainingBladesLayoutBinding.descriptorCount = 1;
    numRemainingBladesLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    numRemainingBladesLayoutBinding.pImmutableSamplers = nullptr;

    std::vector<VkDescriptorSetLayoutBinding> bindings = { inputBladesLayoutBinding, culledBladesLayoutBinding, numRemainingBladesLayoutBinding };
...
void Renderer::CreateComputeDescriptorSets() {
    ...
    for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
        ...
        VkDescriptorBufferInfo numRemainingBladesBufferInfo = {};
        numRemainingBladesBufferInfo.buffer = scene->GetBlades()[i]->GetNumBladesBuffer();
        numRemainingBladesBufferInfo.offset = 0;
        numRemainingBladesBufferInfo.range = sizeof(BladeDrawIndirect);

        uint32_t descriptorWritesIdx = 3 * i;

        descriptorWrites[descriptorWritesIdx].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrites[descriptorWritesIdx].dstSet = computeDescriptorSets[i];
        descriptorWrites[descriptorWritesIdx].dstBinding = 0;
        descriptorWrites[descriptorWritesIdx].dstArrayElement = 0;
        descriptorWrites[descriptorWritesIdx].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        descriptorWrites[descriptorWritesIdx].descriptorCount = 1;
        descriptorWrites[descriptorWritesIdx].pBufferInfo = &inputBladesBufferInfo;
        descriptorWrites[descriptorWritesIdx].pImageInfo = nullptr;
        descriptorWrites[descriptorWritesIdx++].pTexelBufferView = nullptr;

        descriptorWrites[descriptorWritesIdx].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrites[descriptorWritesIdx].dstSet = computeDescriptorSets[i];
        descriptorWrites[descriptorWritesIdx].dstBinding = 1;
        descriptorWrites[descriptorWritesIdx].dstArrayElement = 0;
        descriptorWrites[descriptorWritesIdx].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        descriptorWrites[descriptorWritesIdx].descriptorCount = 1;
        descriptorWrites[descriptorWritesIdx].pBufferInfo = &culledBladesBufferInfo;
        descriptorWrites[descriptorWritesIdx].pImageInfo = nullptr;
        descriptorWrites[descriptorWritesIdx++].pTexelBufferView = nullptr;

        descriptorWrites[descriptorWritesIdx].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        descriptorWrites[descriptorWritesIdx].dstSet = computeDescriptorSets[i];
        descriptorWrites[descriptorWritesIdx].dstBinding = 2;
        descriptorWrites[descriptorWritesIdx].dstArrayElement = 0;
        descriptorWrites[descriptorWritesIdx].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        descriptorWrites[descriptorWritesIdx].descriptorCount = 1;
        descriptorWrites[descriptorWritesIdx].pBufferInfo = &numRemainingBladesBufferInfo;
        descriptorWrites[descriptorWritesIdx].pImageInfo = nullptr;
        descriptorWrites[descriptorWritesIdx].pTexelBufferView = nullptr;
    }
